...
<script>
    ...
    export default {  
        computed: {
            ...
            titles(){
                 let atitles={ id:{title:'ID', typer:'id'},
 ...
 /* . . . . . . . . . . . . . . . . . . . */  
   filterDatasII(){
         ...
         let elem={};
         var nid=0;
        let titles1=this.titles;
         let titlefields = Object.keys(titles1);

        this.brandsdata.forEach(jobj => {
        Object.entries(jobj).forEach(([khey, vall]) => {
            const inkey = titlefields.includes(khey);
            if (inkey==true){
            titlefields.forEach((kk,ii)=>{ 
             
                if ((titles1[kk].typer=="id")&&(nid==0))
        ...
        let tmp=nid;
        let lele=titles1.config.amountcolls;
